...
const App = () => {
    ...
    const [errorMessage, setErrorMessage] = useState('some error hpned...')
    const [posMessage, setPosMessage] = useState('something possitive hpned...')
...
? person
: person.filter(note => note.important)  */}
    return (
      <div>
        <h1>Phonebook</h1>
        <h2>Search</h2>
      ...
      <Phonebook persons = {person} filter = {filter} handleRemove = {handleRemove}/>
       ...
       <Person person = {person} handleRemove = {handleRemove} />{/*toggleImportance = {() => toggleImportance(person.id)}*/}
    <Footer />
      </div>
